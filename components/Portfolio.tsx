    ...
    <section id="portfolio" className={styles.section}>
      <div className={styles.container}>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className={styles.header}>
          ...
          <p className={styles.subtitle}>Projetos mais recentes que desenvolvi ( todos os demais projetos estão disponíveis no meu GitHub )</p>
        </motion.div>
          ...
          {projects.map((project, index) => (
            ...
            >
              <div className={styles.projectImageWrapper}>
                ...
                <motion.div initial={{ opacity: 0 }} whileHover={{ opacity: 1 }} className={styles.projectOverlay}>
                  {project.videoUrl && (
                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => openVideoModal(project.videoUrl!, project.title)} className={styles.overlayButton}>
                      🎥 Demo
                    </motion.button>
                  )}
                  <motion.a whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} href={project.githubUrl} target="_blank" rel="noopener noreferrer" className={styles.overlayButton}>
                    📚 Código
                  </motion.a>
                </motion.div>
              </div>
      ...
      <AnimatePresence>
        {showVideoModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeVideoModal} className={styles.modal}>
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} onClick={(e) => e.stopPropagation()} className={styles.modalContent}>
              <div className={styles.modalHeader}>
                <h3 className={styles.modalTitle}>{currentVideoTitle}</h3>
                <motion.button whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }} onClick={closeVideoModal} className={styles.modalClose}>
                  ×
                </motion.button>
              </div>
              <div className={styles.videoWrapper}>
                <video ref={videoRef} src={currentVideoUrl} controls autoPlay playsInline preload="auto" className={styles.video} key={currentVideoUrl}>
